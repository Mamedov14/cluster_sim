...
class SimConfig:
    # Топология кластера
    num_nodes:           int   = 3       # число рабочих узлов N (Фунг и др., 2025)
    containers_per_node: int   = 3       # контейнеров на узел C (Фунг и др., 2025)
    ...
    base_service_rate:   float = 22.242 # μ₀ = μ(3,1), с⁻¹  (для N=3, C=3)
    ...
    node_mttf_fail:      float = 3500.0   # MTTF аппаратного отказа
...
def execute_all_experiments() -> dict:
    ...
    for mttr in [5, 10, 15, 20, 25]:
        cfg = SimConfig(node_mttr=float(mttr), node_mttf_fail=3500.0, degrade_percent=0.8)
        res = run_replications(cfg)
        res["mttr"] = mttr
...
def generate_plots(data: dict):
    ...
    cfg_fail = "λ=16, Pol=LL, MTTR=15, N=3, C=3, Q=∞"
    cfg_mttr = "λ=16, Pol=LL, Fail=3500, Deg=80%, N=3, C=3, Q=∞"
    cfg_bal  = "λ=16, Fail=3500, Deg=80%, MTTR=15, N=3, C=3, Q=∞"
    cfg_lam  = "Pol=LL, Fail=3500, Deg=80%, MTTR=15, N=3, C=3, Q=∞"
